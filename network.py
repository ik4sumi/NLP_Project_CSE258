...
class MainNet(nn.Module):
  def __init__(self,len_data,dim_in,dim_k):
    ...
    self.MHSA1=MultiHeadSelfAttention(len_data,dim_in,dim_k,dim_k)
    self.MHSA2 = MultiHeadSelfAttention(len_data, dim_in, dim_k, dim_k)
    self.FFN1=FFN(dim_k)
    self.FFN2 = FFN(dim_k)
    self.simple_attention=SimpleAttention(dim_k)
    self.GRU=nn.GRU(dim_in,dim_k,1)
    self.layernorm=nn.LayerNorm(dim_k)
    self.dropout=nn.Dropout(p=0.5)
    self.conv1d=nn.Conv1d(in_channels=dim_in,out_channels=dim_k,kernel_size=1)
    self.transformer=nn.TransformerEncoderLayer(dim_k,2,100,0.1,batch_first=True)
